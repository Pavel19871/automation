...
class EmplTable:
    ...
    def create_table(self):
        with self.__db.connect() as conn:
            conn.execute(self.scripts["create"])
            conn.commit()
    ...
    def insert_employee(self, first_name, last_name, phone, company_id):
        with self.__db.connect() as conn:
            result = conn.execute(self.scripts["insert"],
    ...
    def update_employee(self, employee_id, first_name, last_name, middle_name, phone, email, avatar_url):
        with self.__db.connect() as conn:
            result = conn.execute(self.scripts["update"],
    ...
    def get_employee_by_id(self, employee_id):
        with self.__db.connect() as conn:
            result = conn.execute(self.scripts["get_employee_by_id"], {"employee_id": employee_id})
            return result.fetchone()

    def delete_employee(self, employee_id):
        with self.__db.connect() as conn:
            result = conn.execute(self.scripts["delete"], {"employee_id": employee_id})
            conn.commit()
            return result

    def delete_all_employees(self):
        with self.__db.connect() as conn:
            result = conn.execute(self.scripts["delete_all"])
            conn.commit()
            return result
