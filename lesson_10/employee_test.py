...
def test_get_employee():
    ...
    with allure.step("обращаемся к сотрудникам компании из БД"):
        my_params = str(new_id)  # Преобразуем в строку
...
def test_add_new():
    ...
    with allure.step("cоздаем сотрудника"):
        ...
        new_employee = api.create_employee(first_name, last_name, middle_name, company_id, email, phone)  # Убедитесь, что передаете правильные параметры
    ...
    with allure.step("2-ое обращение к сотрудникам компании через API"):
        ...
        db.delete(id_employee)  # Удаление нового сотрудника
